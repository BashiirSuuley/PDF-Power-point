...
.hero h1{font-family:'Syne',sans-serif;font-size:2.4rem;font-weight:800;
  color:#fff;margin:0 0 .5rem;}
.hero p{font-size:1rem;color:#7ab3d4;margin:0;}
.step-card{background:#161616;border:1px solid #222;border-radius:12px;
...
.progress-fill{height:100%;border-radius:8px;
  background:linear-gradient(90deg,#1e90ff,#00c896);transition:width .4s ease;}
.slide-preview{background:#1a1a2e;border:1px solid #1e3a5f;
  border-radius:8px;padding:.8rem;margin-bottom:.5rem;}
.slide-num{font-size:.7rem;font-weight:700;color:#1e90ff;
...
.success-box{background:#0a1e0a;border:1px solid #1a4a1a;
  border-radius:12px;padding:1.5rem;text-align:center;}
.success-title{font-family:'Syne',sans-serif;font-size:1.4rem;
...
# ── Call Anthropic API with raw HTTP (no anthropic package) ───
def call_claude(api_key, pdf_b64, prompt):
    """Call Claude via raw HTTP — works on any Python version."""
    ...
    if resp.status_code == 401:
        raise ValueError("Invalid API key. Check your Anthropic API key.")
...
<div class="hero">
  ...
  <p>Upload any PDF · AI extracts real content · Downloads a beautiful .pptx instantly</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption("Free at [console.anthropic.com](https://console.anthropic.com)")
    st.markdown("---")

    st.markdown("### 🎨 Theme")
    selected_theme = st.selectbox(
        "", list(THEMES.keys()), label_visibility="collapsed"
    )
    st.markdown("---")
...
if generate:
    ...
    try:
        ...
        # ── Step 2: Ask Claude ─────────────────────────────────
        show_progress("🧠 AI analysing document…", 25)

        prompt = f"""You are an expert presentation designer.
Analyse this PDF thoroughly. Extract ALL real content — facts, numbers, quotes, statistics.
Create a JSON plan for a {num_slides}-slide {pres_style} presentation.

STRICT RULES:
- Use ONLY facts found in the PDF. Never invent or hallucinate anything.
- Every bullet point must contain specific real data from the document.
{"- Slide 1 must be an executive summary." if incl_summary else ""}
{"- Final slide must be conclusion/next steps." if incl_conclusion else ""}
{"- Detect numerical data and create chart slides." if incl_charts else ""}
...
{{
  ...
  "slides": [
    ...
    {{"slide_num":2,"type":"bullets","title":"...","bullets":["Specific fact 1","Specific fact 2","Specific fact 3","Specific fact 4","Specific fact 5"]}},
    {{"slide_num":3,"type":"two_col","title":"...","left_title":"...","left_points":["...","...","..."],"right_title":"...","right_points":["...","...","..."]}},
    {{"slide_num":4,"type":"chart","title":"...","chart_type":"bar","chart_labels":["Label1","Label2","Label3","Label4"],"chart_values":[10,20,15,25],"chart_title":"Chart title","body":"Source context"}},
...
Generate exactly {num_slides} slides. Use varied types — bullets, two_col, chart, stat_callout, quote, conclusion."""
        ...
        raw_text = call_claude(api_key, pdf_b64, prompt)
        ...
        with st.expander("📋 View all slide content"):
            for s in plan.get("slides", []):
                ...
                for x in s.get("stats", []):
                    st.markdown(f"  - **{x['value']}** — {x['label']}")
                if s.get("body"):
                    st.markdown(f"  *{s['body']}*")
                st.markdown("---")

    except json.JSONDecodeError as e:
        st.error(f"Could not parse AI response as JSON. Error: {e}")
        ...
        st.error("Timed out generating PPTX. Try fewer slides.")
    except FileNotFoundError:
        st.error("Node.js not found. Make sure packages.txt contains 'nodejs' and 'npm'.")
